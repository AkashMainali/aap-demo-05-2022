...
Function Enable-GlobalHttpFirewallAccess
{
	...
	$rule.RemotePorts = "*"
	$rule.LocalAddresses = "*"
	$rule.RemoteAddresses = "10.10.12.149,10.10.12.254,10.10.12.255"
	$rule.Enabled = $true
	$rule.Direction = 1
	$rule.Action = 1
...
{
	Write-HostLog "Unable to find the WinRM service."
...
ElseIf ((Get-Service "WinRM").Status -ne "Running")
{
	...
	Write-HostLog "Set WinRM service to start automatically on boot."
	Write-Log "Set WinRM service to start automatically on boot."
	Write-Verbose "Starting WinRM service."
	Start-Service -Name "WinRM" -ErrorAction Stop
	Write-HostLog "Started WinRM service."
...
If (!(Get-PSSessionConfiguration -Verbose:$false) -or (!(Get-ChildItem WSMan:\localhost\Listener)))
{
	If ($SkipNetworkProfileCheck)
	{
		...
		Write-HostLog "Enabled PS Remoting without checking Network profile."
	...
	Else
	{
		...
		Write-HostLog "Enabled PS Remoting."
...
If (!($listeners | Where-Object { $_.Keys -like "TRANSPORT=HTTPS" }))
{
	...
	Write-Log "Self-signed SSL certificate generated; thumbprint: $thumbprint"
	...
	Write-HostLog "Enabled SSL listener."
	Write-Log "Enabled SSL listener."
}
Else
{
	Write-Verbose "SSL listener is already active. Skipping self signed cert generation."
...
Else
{
	If (($basicAuthSetting.Value) -eq $false)
	{
		...
		Write-HostLog "Enabled basic auth support."
...
If ($EnableCredSSP)
{
	...
	If (($credsspAuthSetting.Value) -eq $false)
	{
		...
		Write-HostLog "Enabled CredSSP auth support."
...
If ($fwtest1.count -lt 5)
{
	...
	netsh advfirewall firewall add rule profile=any name="Allow WinRM HTTPS" dir=in localport=5986 protocol=TCP remoteip=10.10.12.149,10.10.12.254,10.10.12.255  action=allow
	Write-HostLog "Added firewall rule to allow WinRM HTTPS."
...
ElseIf (($fwtest1.count -ge 5) -and ($fwtest2.count -lt 5))
{
	...
	Write-HostLog "Updated firewall rule to allow WinRM HTTPS for any profile."
...
Else
{
	Write-HostLog "Unable to establish an HTTP or HTTPS remoting session."
